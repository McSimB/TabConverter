...
public class MidiMessageUtils {
	...
	public static MidiMessage pitchBend(int channel, int value) {
		int MSB = value >> 7;
		int LSB = value - (MSB << 7);
		return MidiMessage.shortMessage(MidiMessage.PITCH_BEND, fixChannel(channel), fixValue(LSB), fixValue(MSB));
	}
	...
	public static MidiMessage trackName(String name) {
		return MidiMessage.metaMessage(MidiMessage.TRACK_NAME, name.getBytes());
	}

	public static MidiMessage endOfTrack() {
		return MidiMessage.metaMessage(MidiMessage.END_OF_TRACK, new byte[]{});
	}
}