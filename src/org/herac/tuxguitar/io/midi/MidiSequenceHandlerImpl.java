...
public class MidiSequenceHandlerImpl extends MidiSequenceHandler {
	...
	public void addNoteOff(long tick, int track, int channelId, int note, int velocity) {
		GMChannelRoute gmChannel = this.router.getRoute(channelId);
		if (gmChannel != null) {
			addEvent(track,
					new MidiEvent(MidiMessageUtils.noteOff(resolveChannel(gmChannel, false), note, 0), tick));
		}
	}

	public void addNoteOn(long tick, int track, int channelId, int note, int velocity) {
		GMChannelRoute gmChannel = this.router.getRoute(channelId);
		if (gmChannel != null) {
			addEvent(track,
					new MidiEvent(MidiMessageUtils.noteOn(resolveChannel(gmChannel, false), note, velocity), tick));
		}
	}

	public void addPitchBend(long tick, int track, int channelId, int value) {
		GMChannelRoute gmChannel = this.router.getRoute(channelId);
		if (gmChannel != null) {
			addEvent(track,
					new MidiEvent(MidiMessageUtils.pitchBend(resolveChannel(gmChannel, false), value), tick));
		}
	}
	...
	public void addControlChange(long tick, int track, int channelId, int controller, int value) {
		...
		if (gmChannel != null) {
			addEvent(track,
					new MidiEvent(MidiMessageUtils.controlChange(gmChannel.getChannel1(), controller, value), tick));
			/*if (gmChannel.getChannel1() != gmChannel.getChannel2()) {
				addEvent(track,
						new MidiEvent(MidiMessageUtils.controlChange(gmChannel.getChannel2(), controller, value), tick));
			}*/
		}
	}
	...
	public void addProgramChange(long tick, int track, int channelId, int instrument) {
		...
		if (gmChannel != null) {
			...
			/*if (gmChannel.getChannel1() != gmChannel.getChannel2()) {
				addEvent(track,
						new MidiEvent(MidiMessageUtils.programChange(gmChannel.getChannel2(), instrument), tick));
			}*/
		}
	}
