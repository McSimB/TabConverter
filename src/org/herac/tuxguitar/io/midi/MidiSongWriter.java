...
public class MidiSongWriter extends MidiFileFormat implements TGSongWriter {
	...
	public void write(TGSongWriterHandle handle) throws TGFileFormatException {
		try {
			...
			MidiSequenceParser midiSequenceParser = new MidiSequenceParser(tgSong, tgSongManager);
			midiSequenceParser.setTranspose(settings.getTranspose());
			midiSequenceParser.parse(new MidiSequenceHandlerImpl((tgSong.countTracks() + 1),
					gmChannelRouter, handle.getOutputStream()));
		} catch (Throwable e) {
