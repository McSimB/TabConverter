...
public class GP1InputStream extends GTPInputStream {
		...
		if ((flags & 0x40) != 0) {
			if (lastReadedStart < start) {
				...
				if (previousBeat != null) {
					List<TGNote> prevNotes = previousBeat.getNotes();
					for (TGNote previous : prevNotes) {
	...
	private void readBeatEffects(TGBeat beat, TGNoteEffect effect) throws IOException {
		...
		beat.getEffect().setFadeIn((flags == 4));
		beat.getEffect().setTapping((flags == 5));
		beat.getEffect().setSlapping((flags == 6));
		beat.getEffect().setPopping((flags == 7));
		if (flags == 3) {
			readBend(effect);
	...
	private void readBend(TGNoteEffect effect) throws IOException {
		...
		bend.addPoint(0f, 0, false);
		bend.addPoint(Math.round(TGEffectBend.MAX_POSITION_LENGTH / 2f),
				Math.round(value * TGEffectBend.SEMITONE_LENGTH), false);
		bend.addPoint(Math.round(TGEffectBend.MAX_POSITION_LENGTH),
				Math.round(value * TGEffectBend.SEMITONE_LENGTH), false);
		effect.setBend(bend);
		skip(1);
	}
	...
	private TGBeat getBeat(TGMeasure measure, long start) {
		if (start >= measure.getStart() && start < (measure.getStart() + measure.getLength())) {
			List<TGBeat> beats = measure.getVoice(0).getBeats();
