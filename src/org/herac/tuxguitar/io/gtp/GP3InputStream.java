...
public class GP3InputStream extends GTPInputStream {
	...
	private int getTiedNoteValue(int string, TGTrack track) {
		...
		if (measureCount > 0) {
			for (int m = measureCount - 1; m >= 0; m--) {
				...
				TGVoice voice = measure.getVoice(0);
				if (voice.countBeats() > 0) {
					for (int b = voice.countBeats() - 1; b >= 0; b--) {
	...
	private long readBeat(long start, TGMeasure measure, TGTrack track, TGTempo tempo) throws IOException {
		...
		TGNoteEffect effect = getFactory().newNoteEffect();
		if ((flags & 0x02) != 0) {
	...
	private int parseRepeatAlternative(TGSong song, int measure, int value) {
		...
		List<TGMeasureHeader> headers = song.getMeasureHeaders();
	...
	private void readBend(TGNoteEffect effect) throws IOException {
		...
		for (int i = 0; i < points; i++) {
			...
			boolean vibrato = readBoolean(); //vibrato
	...
	private void readTremoloBar(TGBeatEffect noteEffect) throws IOException {
		int value = readInt();
		TGEffectTremoloBar effect = getFactory().newEffectTremoloBar();
		effect.addPoint(0f, 0, false);
		effect.addPoint(Math.round(TGEffectTremoloBar.MAX_POSITION_LENGTH / 2f),
				Math.round(-(value / (GP_BEND_SEMITONE * 2f))), false);
		effect.addPoint(TGEffectTremoloBar.MAX_POSITION_LENGTH, 0, false);
	...
	*/
	private void readNoteEffects(TGNoteEffect effect) throws IOException {
		...
		if ((flags & 0x04) != 0) {
			effect.setSlideType(TGNoteEffect.SLIDE_SHIFT_TO);
		}
	...
	*/
	private void readBeatEffects(TGBeat beat, TGNoteEffect noteEffect) throws IOException {
		int flags = readUnsignedByte();
		noteEffect.setVibrato(((flags & 0x01) != 0) || ((flags & 0x02) != 0));
		beat.getEffect().setFadeIn(((flags & 0x10) != 0));
		if ((flags & 0x20) != 0) {
			int type = readUnsignedByte();
			if (type == 0) {
				readTremoloBar(beat.getEffect());
			} else {
				beat.getEffect().setTapping(type == 1);
				beat.getEffect().setSlapping(type == 2);
				beat.getEffect().setPopping(type == 3);
				readInt();
			}
		}
		if ((flags & 0x40) != 0) {
			...
			if (strokeDown > 0) {
				beat.getEffect().getStroke().setDirection(TGStroke.STROKE_DOWN);
				beat.getEffect().getStroke().setValue(toStrokeValue(strokeDown));
			} else if (strokeUp > 0) {
				beat.getEffect().getStroke().setDirection(TGStroke.STROKE_UP);
				beat.getEffect().getStroke().setValue(toStrokeValue(strokeUp));
			}
		}
		if ((flags & 0x04) != 0) {
