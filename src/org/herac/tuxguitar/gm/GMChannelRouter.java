	...
	public void configureRoutesRecursively(GMChannelRoute route, boolean percussionChannel) {
		...
		if( percussionChannel ){
			...
			// Add default routes
				...
				route.setChannel1(( freeChannels.size() > 0 ? freeChannels.get(0) : GMChannelRoute.NULL_VALUE ) );
				route.setChannel2(( freeChannels.size() > 1 ? freeChannels.get(1) : route.getChannel1() ) );
			}
		}
